...
@property (nonatomic, strong) NSMutableArray *stackViewControllers;
@end
...
+ (instancetype)shared{
    ...
    dispatch_once(&onceToken, ^{
        ...
        _instance.stackViewControllers = [NSMutableArray array];
    });
    return _instance;
}
...
- (void)autoLogin{
    ...
    if (user != nil) {
        ...
        [LELoginApi autoLoginComplete:^(NSError * _Nonnull error) {
            if (error !=nil) {
...
- (void)showLoginAlterViewRootViewController:(UIViewController *)viewController withAgreement:(BOOL)agreement withIshiddenCloseView:(BOOL)isHiddenClose{
    LEOALoginControllerView *loginControllerView = [[LEOALoginControllerView alloc] init];
    loginControllerView.modalPresentationStyle = UIModalPresentationCustom;
    loginControllerView.isHiddenCloseView = isHiddenClose;
    loginControllerView.agreement = agreement;
    
    if (![self.stackViewControllers containsObject:@"LEOALoginControllerView"]) {
        [self pushViewController:@"LEOALoginControllerView"];
    }

    // 登录成功完成
    loginControllerView.loginCompleteCallBack = ^(LEUser * _Nonnull user, NSError * _Nonnull error) {
        if (self.loginCompleteCallBack) {
            if (error == nil) {
                [self showFloatViewDashboard:self.viewController];
            }
    ...
    __weak typeof(LEOALoginControllerView *)loginControllerViewWeak = loginControllerView;
    loginControllerView.gotoRegisterEmailViewController = ^{
        
        [self pushViewController:@"LEOAEmailRegisterControllerView"];
        
        // 去注册界面
        [self gotoRegisterEmailViewController:viewController fromViewController:loginControllerViewWeak];
    };
    
    loginControllerView.gotoLoginEmailViewController = ^{
        [self pushViewController:@"LEOAEmailLoginControllerView"];
        // 去登录界面
        [self gotoLoginEmailViewController:viewController fromViewController:loginControllerViewWeak];
    };
    
    
    [viewController presentViewController:loginControllerView animated:NO completion:nil];
}

- (void)gotoRegisterEmailViewController:(UIViewController *)rootViewController fromViewController:(LEOALoginControllerView *)loginControllerView{
    // 去注册界面
    LEOAEmailRegisterControllerView *registerControllerView = [[LEOAEmailRegisterControllerView alloc] init];
    registerControllerView.modalPresentationStyle = UIModalPresentationCustom;

    registerControllerView.registerCompleteCallBack = ^(LEUser * _Nonnull user, NSError * _Nonnull error) {
        if (self.loginCompleteCallBack) {
            if (error == nil) {
                [self showFloatViewDashboard:self.viewController];
            }
    ...
    registerControllerView.closeViewController = ^{
        [self closeALLViewController];
//        [self gotoLoginEmailViewController:rootViewController fromViewController:loginControllerView];
        [self showLoginAlterViewRootViewController:rootViewController withAgreement:YES withIshiddenCloseView:YES];
    };
    registerControllerView.gotoLoginEmailViewController = ^{
        [self pushViewController:@"LEOAEmailLoginControllerView"];
        [self gotoLoginEmailViewController:rootViewController fromViewController:loginControllerView];
    };
    registerControllerView.gotoBackViewController = ^{
      
        [self popViewController:rootViewController fromViewController:loginControllerView];
        
    };
    [rootViewController presentViewController:registerControllerView animated:NO completion:nil];
}

- (void)gotoLoginEmailViewController:(UIViewController *)rootViewController fromViewController:(LEOALoginControllerView *)loginControllerView {
    
//    __strong typeof(LEOALoginControllerView *)loginControllerViewWeak = loginControllerView;
    // 去登录界面
    LEOAEmailLoginControllerView *emailLoginControllerView = [[LEOAEmailLoginControllerView alloc] init];
    emailLoginControllerView.modalPresentationStyle = UIModalPresentationCustom;
    
    emailLoginControllerView.loginEmailCompleteCallBack = ^(LEUser * _Nullable user, NSError * _Nullable error) {
        if (self.loginCompleteCallBack) {
            if (error == nil) {
                [self showFloatViewDashboard:self.viewController];
            }
    ...
    emailLoginControllerView.closeViewController = ^{
        [self closeALLViewController];
//        [self showLoginAlterViewRootViewController:rootViewController withAgreement:YES withIshiddenCloseView:YES];
        [self showLoginAlterViewRootViewController:rootViewController withAgreement:YES withIshiddenCloseView:YES];
    };
    
    emailLoginControllerView.gotoForgetPasswordViewController = ^{
        [self pushViewController:@"LEOAForgetPasswordControllerView"];
        [self gotoForgetPasswordViewController:rootViewController fromViewController:loginControllerView];
    };
    

    emailLoginControllerView.gotoRegisterViewController = ^{
//        [self.stackViewControllers addObject:@"LEOAEmailRegisterControllerView"];
        if ([self.stackViewControllers containsObject:@"LEOAEmailRegisterControllerView"]) {
            [self popViewController:rootViewController fromViewController:loginControllerView];
        } else {
            [self pushViewController:@"LEOAEmailRegisterControllerView"];
            [self gotoRegisterEmailViewController:rootViewController fromViewController:loginControllerView];
        }
    ...
    emailLoginControllerView.gotoBackViewController = ^{
      
        [self popViewController:rootViewController fromViewController:loginControllerView];
        
    };
    [rootViewController presentViewController:emailLoginControllerView animated:NO completion:nil];
}

- (void)gotoForgetPasswordViewController:(UIViewController *)rootViewController fromViewController:(LEOALoginControllerView *)loginControllerView {
//    __strong typeof(LEOALoginControllerView *)loginControllerViewWeak = loginControllerView;
    // 去登录界面
    LEOAForgetPasswordControllerView *forgetPasswordController = [[LEOAForgetPasswordControllerView alloc] init];
    forgetPasswordController.modalPresentationStyle = UIModalPresentationCustom;
    forgetPasswordController.closeViewController = ^{
//        [self gotoLoginEmailViewController:rootViewController fromViewController:loginControllerView];
        [self closeALLViewController];
        [self showLoginAlterViewRootViewController:rootViewController withAgreement:YES withIshiddenCloseView:YES];
    };
    forgetPasswordController.gotoLoginEmailViewController = ^{
//        [self.stackViewControllers addObject:@"LEOAForgetPasswordControllerView"];
//        [self gotoLoginEmailViewController:rootViewController fromViewController:loginControllerView];
        [self popViewController:rootViewController fromViewController:loginControllerView];
    };
    forgetPasswordController.gotoBackViewController = ^{
      
        [self popViewController:rootViewController fromViewController:loginControllerView];
        
    };
    [rootViewController presentViewController:forgetPasswordController animated:NO completion:nil];
...
//    NSString *first  = self.stackViewControllers.firstObject;
    [self.stackViewControllers removeAllObjects];
}

- (void)pushViewController:(NSString *)name{
    NSLog(@"push before= %@", self.stackViewControllers);
    [self.stackViewControllers addObject:name];
    NSLog(@"push after= %@", self.stackViewControllers);
}

- (NSString*)getViewControllerName:(int)index{
    
    NSString *name = nil;
    if (self.stackViewControllers.count > index) {
        name = self.stackViewControllers[index];
    }
    return name;
    
}

- (void)popViewController:(UIViewController *)rootViewController fromViewController:(LEOALoginControllerView *)loginControllerView{
    NSLog(@"pop before= %@", self.stackViewControllers);
    if (self.stackViewControllers.count > 1) {
        [self.stackViewControllers removeLastObject];
    }
   
    NSString *viewControllerStr = self.stackViewControllers.lastObject;
    NSLog(@"pop after= %@", self.stackViewControllers);
    if ([viewControllerStr isEqualToString:@"LEOAForgetPasswordControllerView"]) {
        [self gotoForgetPasswordViewController:rootViewController fromViewController:loginControllerView];
    } else if([viewControllerStr isEqualToString:@"LEOAEmailRegisterControllerView"]){
        [self gotoRegisterEmailViewController:rootViewController fromViewController:loginControllerView];
    }else if([viewControllerStr isEqualToString:@"LEOAEmailLoginControllerView"]){
        [self gotoLoginEmailViewController:rootViewController fromViewController:loginControllerView];
    }else if([viewControllerStr isEqualToString:@"LEOAForgetPasswordControllerView"]){
        [self gotoForgetPasswordViewController:rootViewController fromViewController:loginControllerView];
    }else if([viewControllerStr isEqualToString:@"LEOALoginControllerView"]){
        [self showLoginAlterViewRootViewController:rootViewController withAgreement:YES withIshiddenCloseView:YES];
    }
...
- (void)showBindingAccountCenterController:(UIViewController *)viewController{
    LEOABindControllerView *bindControllerView = [[LEOABindControllerView alloc] init];
    bindControllerView.modalPresentationStyle = UIModalPresentationCustom;
    bindControllerView.gotoBindingEmailViewController = ^{
        [self gotoBindingEmailViewController:viewController];
    };
    [viewController presentViewController:bindControllerView animated:NO completion:nil];
}

- (void)gotoBindingEmailViewController:(UIViewController *)rootViewController{
    
    LEOABindEmailControllerView *bindEmailControllerView = [[LEOABindEmailControllerView alloc] init];
    bindEmailControllerView.modalPresentationStyle = UIModalPresentationCustom;
    bindEmailControllerView.closeViewController = ^{
        [self showBindingAccountCenterController:rootViewController];
    };
    [rootViewController presentViewController:bindEmailControllerView animated:NO completion:nil];
}
