...
@interface LEOAEmailRegisterView ()
@property (nonatomic,weak) UIView *contentView;
@end

@implementation LEOAEmailRegisterView
...
+ (instancetype)instanceOAEmailRegisterView{
    ...
    LEOAEmailRegisterView *view = [[bundle loadNibNamed:@"LEOAEmailRegisterView" owner:nil options:nil] firstObject];
    ...
    view.label_TopTip.text = [NSBundle le_localizedStringForKey:@"Create an account"];
    ...
    view.label_BottomTip.text = [NSBundle le_localizedStringForKey:@"Already have an account"];
    [view.button_SingIn setTitle:[NSBundle le_localizedStringForKey:@"Sing in"] forState:UIControlStateNormal];
...
- (IBAction)registerEmailAccount:(UIButton *)sender {
    ...
    if (self.registerEmailEvent) {
        self.registerEmailEvent(sender, self.textField_Email.text, self.textField_Code.text, self.textField_Password.text);
    }
}

- (IBAction)gotoLoginEmail:(UIButton *)sender {
    if (self.loginEmailEvent) {
        self.loginEmailEvent(sender);
    }
}
